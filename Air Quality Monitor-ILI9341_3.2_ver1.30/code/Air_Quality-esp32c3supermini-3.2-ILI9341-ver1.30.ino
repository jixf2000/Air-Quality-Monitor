...
void initClockStaticUI() {
  ...
  // 环境参数标签
  displayChineseCenterSmall(GRID_L, GRID_MID_X, TOP_LABEL_Y, "Humidity", ILI9341_YELLOW, UI_BG);
  displayChineseCenterSmall(GRID_MID_X, GRID_R, TOP_LABEL_Y, "Temperature", ILI9341_YELLOW, UI_BG);
  displayChineseCenterSmall(GRID_L, GRID_MID_X, TVOC_LABEL_Y, "TVOC", ILI9341_YELLOW, UI_BG);
  displayChineseCenterSmall(GRID_MID_X, GRID_R, CO2_LABEL_Y, "CO2", ILI9341_YELLOW, UI_BG);
...
void drawEnvDynamic(float temp, float hum, uint16_t tvoc, uint16_t eco2) {
  uint16_t colHUMI = UI_BLUE;
  uint16_t colTEMP = UI_BLUE;
  uint16_t colTVOC = UI_BLUE;
  uint16_t colCO2  = UI_BLUE;
...
void initEnvironmentStaticUI() {
  ...
  // 左上区域 - 湿度
  displayChineseCenterSmooth(0, 160, 32, "Humidity", ILI9341_YELLOW, UI_BG, false);
  
  // 右上区域 - 温度
  displayChineseCenterSmooth(160, 320, 32, "Temperature", ILI9341_YELLOW, UI_BG, false);
  
  // 左下区域 - TVOC
  displayChineseCenterSmooth(0, 160, 152, "TVOC", ILI9341_YELLOW, UI_BG, false);
  
  // 右下区域 - CO2
  displayChineseCenterSmooth(160, 320, 152, "CO2", ILI9341_YELLOW, UI_BG, false);
...
void drawEnvQuadrant(float humidity, float temperature, uint16_t tvoc, uint16_t eco2) {
  ...
  displayChineseCenterSmooth(0, 160, 80, humBuf, UI_BLUE, UI_BG, true);  // 使用大字体显示数值
  ...
  displayChineseCenterSmooth(160, 320, 80, tempBuf, UI_BLUE, UI_BG, true);  // 使用大字体显示数值
  ...
  displayChineseCenterSmooth(0, 160, 200, tvocBuf, UI_BLUE, UI_BG, true);  // 使用大字体显示数值
  ...
  displayChineseCenterSmooth(160, 320, 200, eco2Buf, UI_BLUE, UI_BG, true);  // 使用大字体显示数值
...
void loadUIColorConfig() {
  ...
  UI_ACCENT = (uint16_t)preferences.getUInt("UI_ACCENT", DEFAULT_ACCENT);
  UI_BLUE = (uint16_t)preferences.getUInt("UI_BLUE", DEFAULT_BLUE);
  UI_GREEN = (uint16_t)preferences.getUInt("UI_GREEN", DEFAULT_GREEN);
  UI_PINK = (uint16_t)preferences.getUInt("UI_PINK", DEFAULT_PINK);
  UI_DARK = (uint16_t)preferences.getUInt("UI_DARK", DEFAULT_DARK);
  UI_LIGHT_GREY = (uint16_t)preferences.getUInt("UI_LIGHT_GREY", DEFAULT_LIGHT_GREY);
  UI_DARKER_GREY = (uint16_t)preferences.getUInt("UI_DARKER_GREY", DEFAULT_DARKER_GREY);
  
  Serial.println("=== Loaded UI color configuration ===");
  preferences.end();
}
...
void saveUIColorConfig() {
  ...
  preferences.putUInt("UI_ACCENT", (uint32_t)UI_ACCENT);
  preferences.putUInt("UI_BLUE", (uint32_t)UI_BLUE);
  preferences.putUInt("UI_GREEN", (uint32_t)UI_GREEN);
  preferences.putUInt("UI_PINK", (uint32_t)UI_PINK);
  preferences.putUInt("UI_DARK", (uint32_t)UI_DARK);
  preferences.putUInt("UI_LIGHT_GREY", (uint32_t)UI_LIGHT_GREY);
  preferences.putUInt("UI_DARKER_GREY", (uint32_t)UI_DARKER_GREY);
  
  Serial.println("=== Saved UI color configuration ===");
  preferences.end();
}
...
void sendTempCalibrationPage() {
  ...
  html += "<label for='ui_accent'>Accent Color (HEX):</label>";
  html += "<input type='color' id='ui_accent' name='ui_accent' value='" + String(rgb565ToHex(UI_ACCENT)) + "'><br>";
  html += "<label for='ui_blue'>Blue Color (HEX):</label>";
  html += "<input type='color' id='ui_blue' name='ui_blue' value='" + String(rgb565ToHex(UI_BLUE)) + "'><br>";
  html += "<label for='ui_green'>Green Color (HEX):</label>";
  html += "<input type='color' id='ui_green' name='ui_green' value='" + String(rgb565ToHex(UI_GREEN)) + "'><br>";
  html += "<label for='ui_pink'>Pink Color (HEX):</label>";
  html += "<input type='color' id='ui_pink' name='ui_pink' value='" + String(rgb565ToHex(UI_PINK)) + "'><br>";
  html += "<label for='ui_dark'>Dark Color (HEX):</label>";
  html += "<input type='color' id='ui_dark' name='ui_dark' value='" + String(rgb565ToHex(UI_DARK)) + "'><br>";
  html += "<label for='ui_light_grey'>Light Grey Color (HEX):</label>";
  html += "<input type='color' id='ui_light_grey' name='ui_light_grey' value='" + String(rgb565ToHex(UI_LIGHT_GREY)) + "'><br>";
  html += "<label for='ui_darker_grey'>Darker Grey Color (HEX):</label>";
  html += "<input type='color' id='ui_darker_grey' name='ui_darker_grey' value='" + String(rgb565ToHex(UI_DARKER_GREY)) + "'><br>";
...
void handleUIColorConfig() {
  ...
  String accentStr = backgroundServer.arg("ui_accent");
  String blueStr = backgroundServer.arg("ui_blue");
  String greenStr = backgroundServer.arg("ui_green");
  String pinkStr = backgroundServer.arg("ui_pink");
  String darkStr = backgroundServer.arg("ui_dark");
  String lightGreyStr = backgroundServer.arg("ui_light_grey");
  String darkerGreyStr = backgroundServer.arg("ui_darker_grey");
  
  Serial.printf("Received UI colors: bg=%s, light=%s, accent=%s\n", 
                bgStr.c_str(), lightStr.c_str(), accentStr.c_str());
  
  // 更新UI颜色配置
  UI_BG = hexToRgb565(bgStr);
  UI_LIGHT = hexToRgb565(lightStr);
  UI_ACCENT = hexToRgb565(accentStr);
  UI_BLUE = hexToRgb565(blueStr);
  UI_GREEN = hexToRgb565(greenStr);
  UI_PINK = hexToRgb565(pinkStr);
  UI_DARK = hexToRgb565(darkStr);
  UI_LIGHT_GREY = hexToRgb565(lightGreyStr);
  UI_DARKER_GREY = hexToRgb565(darkerGreyStr);
