...
#define KEY0_PIN     0
#define KEY1_PIN     10  // 新增按键引脚，用于切换页面
...
#define CYBER_LIGHT   ST77XX_BLACK  
...
void displayChineseSmooth(int x, int y, const char* text) {
  displayChineseSmooth(x, y, text, ST77XX_BLACK, CYBER_BG, false);
}
...
void displayChineseCenterSmooth(int x0, int x1, int y, const char* text) {
  displayChineseCenterSmooth(x0, x1, y, text, ST77XX_BLACK, CYBER_BG, false);
}

void displayChineseCenter(int x0, int x1, int y, const char* text, uint16_t color = ST77XX_BLACK, uint16_t bgColor = CYBER_BG) {
  // 使用平滑字体版本
  displayChineseCenterSmooth(x0, x1, y, text, color, bgColor, false);
}

// 专门用于小字体标签的函数
void displayChineseCenterSmall(int x0, int x1, int y, const char* text, uint16_t color = ST77XX_BLACK, uint16_t bgColor = CYBER_BG) {
  tft.setFont(&FreeSans9pt7b);
...
void drawMenu() {
  ...
  for (int i = 0; i < MENU_ITEMS; i++) {
    ...
    if (i == menuIndex) {
      ...
      displayChineseSmooth(24, y, menuItemsEN[i], ST77XX_BLACK);
    }
  }
  // drawAlarmIcon(); // 闹钟功能已移除
}
...
void drawAlarmScreen(bool full) {
  // 闹钟功能已被移除，此函数为空实现
  tft.fillScreen(CYBER_BG);
  displayChineseSmooth(20, 100, "Feature Removed", ST77XX_BLACK);
}
...
void initEnvironmentStaticUI() {
  ...
  // 左上区域 - 湿度
  displayChineseCenterSmooth(0, 160, 32, "Humidity", ST77XX_YELLOW, CYBER_BG, false);
  
  // 右上区域 - 温度
  displayChineseCenterSmooth(160, 320, 32, "Temperature", ST77XX_YELLOW, CYBER_BG, false);
  
  // 左下区域 - TVOC
  displayChineseCenterSmooth(0, 160, 152, "TVOC", ST77XX_YELLOW, CYBER_BG, false);
  
  // 右下区域 - CO2
  displayChineseCenterSmooth(160, 320, 152, "CO2", ST77XX_YELLOW, CYBER_BG, false);
...
void setup() {
  ...
  Wire.begin(SDA_PIN, SCL_PIN);
  SPI.begin(TFT_SCLK, TFT_MISO, TFT_MOSI, TFT_CS);  // 为ST7789配置SPI引脚

  tft.init(240, 320); // 初始化ST7789，分辨率为240x320
  tft.setRotation(1);
  tft.fillScreen(CYBER_BG);
