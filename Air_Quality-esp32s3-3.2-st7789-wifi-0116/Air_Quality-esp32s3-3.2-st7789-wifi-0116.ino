...
// ====== 颜色定义 ======
#define CYBER_BG      ST77XX_WHITE
#define CYBER_GREEN   ST77XX_GREEN  
#define CYBER_ACCENT  ST77XX_CYAN  
#define CYBER_LIGHT   0xFD20  
#define CYBER_BLUE    ST77XX_BLUE  
#define CYBER_PINK    ST77XX_MAGENTA
#define CYBER_LIGHT_GREY 0xC618      // 浅灰色 (RGB: 198, 201, 207 -> 0xC618)

#define AQ_BAR_GREEN  ST77XX_GREEN    // 0x07E0
#define AQ_BAR_YELLOW ST77XX_YELLOW   // 0xFFE0
#define AQ_BAR_ORANGE 0xFD20            // 自定义橙色
#define AQ_BAR_RED    ST77XX_RED      // 0xF800
...
unsigned long lastEnvDisplayUpdate = 0;  // 记录上次环境变量显示更新时间
...
void displayChineseSmooth(int x, int y, const char* text) {
  displayChineseSmooth(x, y, text, ST77XX_WHITE, CYBER_BG, false);
}
...
void displayChineseCenterSmooth(int x0, int x1, int y, const char* text) {
  displayChineseCenterSmooth(x0, x1, y, text, ST77XX_WHITE, CYBER_BG, false);
}

void displayChineseCenter(int x0, int x1, int y, const char* text, uint16_t color = ST77XX_WHITE, uint16_t bgColor = CYBER_BG) {
  // 使用平滑字体版本
  displayChineseCenterSmooth(x0, x1, y, text, color, bgColor, false);
}

// 专门用于小字体标签的函数
void displayChineseCenterSmall(int x0, int x1, int y, const char* text, uint16_t color = ST77XX_WHITE, uint16_t bgColor = CYBER_BG) {
  tft.setFont(&FreeSans9pt7b);
...
void connectWiFiAndSyncTimeOriginal() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    tft.fillScreen(CYBER_BG);
    displayChineseSmooth(20, 110, "WiFi Connect Failed!", ST77XX_RED);
    delay(1000);
  }
}
...
const int GRID_L   = 52;
const int GRID_R   = 288;
const int GRID_TOP = 80;
const int GRID_MID = 160;
const int GRID_BOT = 240;
const int GRID_MID_X = (GRID_L + GRID_R) / 2;

const int TOP_LABEL_Y      = GRID_TOP + 25;  // 向下移动23个像素
const int TOP_VALUE_Y      = GRID_TOP + 59;  // 向下移动25个像素
const int BOTTOM_LABEL_Y   = GRID_MID + 12;
const int BOTTOM_VALUE_Y   = GRID_MID + 34;
const int TVOC_LABEL_Y     = GRID_MID + 14;  // 向上移动10个像素
const int TVOC_VALUE_Y     = GRID_MID + 40;
const int CO2_LABEL_Y      = GRID_MID + 14;  // 向上移动10个像素
const int CO2_VALUE_Y      = GRID_MID + 40;

const int BAR_MARGIN_X = 52;
const int BAR_GAP      = 2;
const int BAR_Y        = 300;
const int BAR_H        = 12;
const int BAR_W        = (240 - 2 * BAR_MARGIN_X - 3 * BAR_GAP) / 4;
...
void initClockStaticUI() {
  ...
  displayChineseCenterSmooth(0, 240, 16, "Environment Monitor", CYBER_LIGHT);
  // displayChineseSmooth(8, 88, "Air Quality:", ST77XX_BLACK);  // 空气质量标签（已隐藏）
  ...
  // 环境参数标签
  displayChineseCenterSmall(GRID_L, GRID_MID_X, TOP_LABEL_Y, "Humidity", ST77XX_YELLOW, CYBER_BG);
  displayChineseCenterSmall(GRID_MID_X, GRID_R, TOP_LABEL_Y, "Temperature", ST77XX_YELLOW, CYBER_BG);
  displayChineseCenterSmall(GRID_L, GRID_MID_X, TVOC_LABEL_Y, "TVOC", ST77XX_YELLOW, CYBER_BG);
  displayChineseCenterSmall(GRID_MID_X, GRID_R, CO2_LABEL_Y, "CO2", ST77XX_YELLOW, CYBER_BG);
...
void drawClockTime(String hourStr, String minStr, String secStr) {
  ...
  if (prevTimeStr != "") {
    ...
    int oldX = (240 - adjustedWidth) / 2;
    // 增加上部范围和整体额外宽度以确保完全清除旧文本（缩小上下范围）
    tft.fillRect(oldX + x1, 20, adjustedWidth + 35, h + 30, CYBER_BG);  // 再向右扩大5个像素
  }
  ...
  int baseX = (240 - totalWidth) / 2 + 50;  // 使用总宽度计算居中位置，并向右偏移50像素
  ...
  tft.drawFastHLine(52, lineY, 236, CYBER_LIGHT_GREY);  // 绘制水平分割线
...
void drawMenu() {
  ...
  displayChineseCenterSmooth(0, 240, 20, "Mode Select", CYBER_LIGHT);
  ...
  for (int i = 0; i < MENU_ITEMS; i++) {
    ...
    if (i == menuIndex) {
      ...
      uint16_t w1, h1;
      tft.setFont(&FreeSans12pt7b);
      tft.getTextBounds(menuItemsEN[i], 0, 0, &x1, &y1, &w1, &h1);
      int itemX = (240 - w1) / 2;
      tft.fillRect(itemX - 4, y - 14, w1 + 8, 28, CYBER_ACCENT);
      displayChineseSmooth(itemX, y, menuItemsEN[i], CYBER_BG, CYBER_ACCENT);
      tft.setFont();
    } else {
      // 居中对齐未选中的菜单项
      int16_t x2, y2;
      uint16_t w2, h2;
      tft.setFont(&FreeSans12pt7b);
      tft.getTextBounds(menuItemsEN[i], 0, 0, &x2, &y2, &w2, &h2);
      int itemX = (240 - w2) / 2;
      tft.fillRect(itemX - 4, y - 14, w2 + 8, 28, CYBER_BG);
      displayChineseSmooth(itemX, y, menuItemsEN[i], ST77XX_BLACK);
      tft.setFont();
    }
  }
  // drawAlarmIcon(); // 闹钟功能已移除
...
void drawAlarmScreen(bool full) {
  // 闹钟功能已被移除，此函数为空实现
  tft.fillScreen(CYBER_BG);
  displayChineseSmooth(12, 100, "Feature Removed", ST77XX_BLACK);
}
...
void initEnvironmentStaticUI() {
  ...
  // 绘制十字分割线 - 细的浅灰色线条
  // 绘制水平分割线
  tft.drawFastHLine(50, 120, 240, CYBER_LIGHT_GREY);  // 使用细线绘制浅灰色水平线
  // 绘制垂直分割线
  tft.drawFastVLine(170, 0, 320, CYBER_LIGHT_GREY);  // 将垂直线位置向右移动50像素  // 使用细线绘制浅灰色垂直线
  
  // 左上区域 - 湿度
  displayChineseCenterSmooth(35, 155, 40, "Humidity", ST77XX_YELLOW, CYBER_BG, false);
    
  // 右上区域 - 温度
  displayChineseCenterSmooth(180, 300, 40, "Temperature", ST77XX_YELLOW, CYBER_BG, false);
  
  // 左下区域 - TVOC
  displayChineseCenterSmooth(50, 170, 160, "TVOC", ST77XX_YELLOW, CYBER_BG, false);
  
  // 右下区域 - CO2
  displayChineseCenterSmooth(170, 290, 160, "CO2", ST77XX_YELLOW, CYBER_BG, false);
...
void drawEnvQuadrant(float humidity, float temperature, uint16_t tvoc, uint16_t eco2) {
  ...
  // 先清除旧的数值区域，再显示新数值
  tft.fillRect(50, 55, 120, 82, CYBER_BG);  // 清除湿度值区域（向上移动8个像素）
  displayChineseCenterSmooth(40, 160, 105, humBuf, CYBER_BLUE, CYBER_BG, true);  // 使用大字体显示数值
  
  // 右上区域 - 温度值
  char tempBuf[10];
  sprintf(tempBuf, "%.1fC", temperature);
  // 先清除旧的数值区域，再显示新数值
  tft.fillRect(170, 55, 120, 82, CYBER_BG);  // 清除温度值区域（向上移动8个像素）
  displayChineseCenterSmooth(178, 298, 105, tempBuf, CYBER_BLUE, CYBER_BG, true);  // 使用大字体显示数值
  
  // 左下区域 - TVOC值
  char tvocBuf[12];
  sprintf(tvocBuf, "%d ppb", tvoc);
  // 先清除旧的数值区域，再显示新数值
  tft.fillRect(50, 170, 120, 70, CYBER_BG);  // 清除TVOC值区域（继续改小5个像素）
  displayChineseCenterSmooth(40, 160, 200, tvocBuf, CYBER_BLUE, CYBER_BG, true);  // 使用大字体显示数值
  
  // 右下区域 - CO2值
  char eco2Buf[12];
  sprintf(eco2Buf, "%d ppm", eco2);
  // 先清除旧的数值区域，再显示新数值
  tft.fillRect(170, 170, 120, 70, CYBER_BG);  // 清除CO2值区域（继续改小5个像素）
  displayChineseCenterSmooth(178, 298, 200, eco2Buf, CYBER_BLUE, CYBER_BG, true);  // 使用大字体显示数值
}
...
void drawDateTimePage() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    int16_t x_ip, y_ip;
    uint16_t w_ip, h_ip;
    tft.getTextBounds(ipStr, 0, 0, &x_ip, &y_ip, &w_ip, &h_ip);
    int ipX = (240 - w_ip) / 2 + 50;  // 向右偏移50像素
    tft.setCursor(ipX, 160);
    tft.print(ipStr);
    
    // 显示配置页面地址和端口
    String configStr1 = "Config: ";
    int16_t x_c1, y_c1;
    uint16_t w_c1, h_c1;
    tft.getTextBounds(configStr1, 0, 0, &x_c1, &y_c1, &w_c1, &h_c1);
    int configX1 = (240 - w_c1) / 2 + 50;  // 向右偏移50像素
    tft.setCursor(configX1, 180);
    tft.print(configStr1);
    
    String configStr2 = WiFi.localIP().toString() + ":8080";
    int16_t x_c2, y_c2;
    uint16_t w_c2, h_c2;
    tft.getTextBounds(configStr2, 0, 0, &x_c2, &y_c2, &w_c2, &h_c2);
    int configX2 = (240 - w_c2) / 2 + 50;  // 向右偏移50像素
    tft.setCursor(configX2, 200);
    tft.print(configStr2);
    
    // 显示版本信息和作者邮箱
    String versionStr = "v1.0 7591196@qq.com";
    int16_t x_v, y_v;
    uint16_t w_v, h_v;
    tft.getTextBounds(versionStr, 0, 0, &x_v, &y_v, &w_v, &h_v);
    int versionX = (240 - w_v) / 2 + 50;  // 向右偏移50像素
    tft.setCursor(versionX, 220);
...
void initHistoryStaticUI() {
  ...
  // 绘制坐标轴
  tft.drawLine(40, 40, 40, 220, ST77XX_BLACK);  // Y轴
  tft.drawLine(40, 220, 300, 220, ST77XX_BLACK);  // X轴
  ...
  displayChineseSmooth(250, 60, "Temp", ST77XX_BLACK);
  
  tft.fillRect(240, 80, 8, 8, CYBER_GREEN);     // 湿度
  displayChineseSmooth(250, 80, "Humi", ST77XX_BLACK);
  
  tft.fillRect(240, 100, 8, 8, CYBER_PINK);    // TVOC
  displayChineseSmooth(250, 100, "TVOC", ST77XX_BLACK);
  
  tft.fillRect(240, 120, 8, 8, CYBER_ACCENT);   // CO2
  displayChineseSmooth(250, 120, "CO2", ST77XX_BLACK);
}
...
void drawHistoryGraphs() {
  ...
  if (validPoints > 1) {
    ...
    // 如果没有足够的数据点，显示提示信息
    displayChineseSmooth(100, 120, "No Data Yet", ST77XX_RED);
  }
...
void setup() {
  ...
  tft.init(240, 320); // 初始化ST7789，指定分辨率为240x320
  
  // ST7789V完整初始化序列
  // 复位序列
  tft.enableDisplay(true); // 开启显示
  delay(100);
  
  // 显示设置
  tft.sendCommand(0x36, (uint8_t[]){0x00}, 1);
  tft.sendCommand(0x3a, (uint8_t[]){0x05}, 1);
  
  // 帧率设置
  tft.sendCommand(0xb2, (uint8_t[]){0x0c, 0x0c, 0x00, 0x33, 0x33}, 5);
  tft.sendCommand(0xb7, (uint8_t[]){0x35}, 1);
  
  // 电源设置
  tft.sendCommand(0xbb, (uint8_t[]){0x20}, 1);
  tft.sendCommand(0xc0, (uint8_t[]){0x2c}, 1);
  tft.sendCommand(0xc2, (uint8_t[]){0x01}, 1);
  tft.sendCommand(0xc3, (uint8_t[]){0x0b}, 1);
  tft.sendCommand(0xc4, (uint8_t[]){0x20}, 1);
  tft.sendCommand(0xc6, (uint8_t[]){0x0f}, 1);
  tft.sendCommand(0xd0, (uint8_t[]){0xa4, 0xa1}, 2);
  
  tft.writeCommand(0x11); // Sleep Out
  delay(120);
  
  tft.setRotation(3);  // 设置屏幕旋转角度为3，实现右旋90度
  tft.fillScreen(CYBER_BG);
...
void loop() {
  ...
  switch (currentMode) {
    ...
    case MODE_CLOCK: {
      ...
      // 在时钟模式下也定期更新环境数据显示，保持与环境参数四等分显示模式的一致性
      // 同时更新第一页专用温度值
      first_page_temperature = current_env_data.temperature;
      // 每10秒更新一次环境数据显示
      unsigned long currentTime = millis();
      if (currentTime - lastEnvDisplayUpdate >= 10000) {  // 10秒更新一次显示
        drawEnvDynamic(current_env_data.temperature, current_env_data.humidity, current_env_data.tvoc, current_env_data.eco2);
        lastEnvDisplayUpdate = currentTime;
      }
...
void connectWiFiAndSyncTime() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (!timeSyncSuccess) {
      ...
      displayChineseSmooth(20, 150, "Time Sync Timeout", ST77XX_RED);
    ...
    tft.fillScreen(CYBER_BG);
    displayChineseSmooth(20, 110, "WiFi Config Failed!", ST77XX_RED);
...
void handleConfig() {
  ...
  if (ssid.length() > 0) {
    ...
    if (connectWiFi(ssid, password)) {
      ...
      successHtml += "<p>Device will restart to apply settings</p>";
      successHtml += "</body></html>";
