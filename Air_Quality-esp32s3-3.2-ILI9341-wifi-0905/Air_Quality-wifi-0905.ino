...
const int TOP_LABEL_Y      = GRID_TOP + 2;  // 向上移动10个像素
...
void drawClockTime(String hourStr, String minStr, String secStr) {
  ...
  if (prevTimeStr != "") {
    ...
    tft.fillRect(oldX + x1 - 15, 20, adjustedWidth + 30, h + 30, CYBER_BG);
  }
...
void initEnvironmentStaticUI() {
  ...
  // 左上区域 - 湿度
  displayChineseCenterSmooth(0, 160, 40, "Humidity", ILI9341_YELLOW, CYBER_BG, false);
  
  // 右上区域 - 温度
  displayChineseCenterSmooth(160, 320, 40, "Temperature", ILI9341_YELLOW, CYBER_BG, false);
  
  // 左下区域 - TVOC
  displayChineseCenterSmooth(0, 160, 160, "TVOC", ILI9341_YELLOW, CYBER_BG, false);
  
  // 右下区域 - CO2
  displayChineseCenterSmooth(160, 320, 160, "CO2", ILI9341_YELLOW, CYBER_BG, false);
...
void drawEnvQuadrant(float humidity, float temperature, uint16_t tvoc, uint16_t eco2) {
  ...
  // 先清除旧的数值区域，再显示新数值
  tft.fillRect(0, 40, 160, 80, CYBER_BG);  // 清除湿度值区域（扩大）
  ...
  // 先清除旧的数值区域，再显示新数值
  tft.fillRect(160, 40, 160, 80, CYBER_BG);  // 清除温度值区域（扩大）
  ...
  // 先清除旧的数值区域，再显示新数值
  tft.fillRect(0, 160, 160, 80, CYBER_BG);  // 清除TVOC值区域（扩大）
  ...
  // 先清除旧的数值区域，再显示新数值
  tft.fillRect(160, 160, 160, 80, CYBER_BG);  // 清除CO2值区域（扩大）
