...
void displayChinese(int x, int y, const char* text, uint16_t color = ST77XX_WHITE, uint16_t bgColor = CYBER_BG) {
...
void connectWiFiAndSyncTimeOriginal() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    tft.fillScreen(CYBER_BG);
    displayChinese(20, 110, "WiFi Connect Failed!", ST77XX_RED);
    delay(1000);
  }
}
...
void initClockStaticUI() {
  ...
  displayChinese(8, 88, "Air Quality:", ST77XX_BLACK);
  ...
  displayChineseCenter(GRID_L, GRID_MID_X, TOP_LABEL_Y, "Humidity", ST77XX_BLACK);
  displayChineseCenter(GRID_MID_X, GRID_R, TOP_LABEL_Y, "Temperature", ST77XX_BLACK);
  displayChineseCenter(GRID_L, GRID_MID_X, BOTTOM_LABEL_Y, "TVOC", ST77XX_BLACK);
  displayChineseCenter(GRID_MID_X, GRID_R, BOTTOM_LABEL_Y, "CO2", ST77XX_BLACK);
...
void drawMenu() {
  ...
  for (int i = 0; i < MENU_ITEMS; i++) {
    ...
    if (i == menuIndex) {
      ...
      displayChinese(24, y, menuItemsEN[i], ST77XX_BLACK);
    }
  }
  drawAlarmIcon();
}
...
void drawAlarmScreen(bool full) {
  ...
  tft.setCursor(x, y);
  tft.setTextColor(alarmSelectedField == 0 ? CYBER_LIGHT : ST77XX_BLACK, CYBER_BG);
  tft.print(hBuf);

  tft.setTextColor(ST77XX_BLACK, CYBER_BG);
  tft.print(":");

  tft.setTextColor(alarmSelectedField == 1 ? CYBER_LIGHT : ST77XX_BLACK, CYBER_BG);
  tft.print(mBuf);
  ...
  displayChinese(60, 168, "Alarm:", ST77XX_BLACK);
...
void drawAlarmRingingScreen() {
  ...
  displayChineseCenter(0, 320, 80, "Alarm Ringing!", ST77XX_WHITE, ST77XX_RED);
}
...
void initAPMode() {
  ...
  tft.fillScreen(CYBER_BG);
  displayChinese(20, 60, "AP Config Mode", ST77XX_BLACK);
  
  String ssidText = "SSID: " + String(AP_SSID);
  String pwdText = "PWD: " + String(AP_PASSWORD);
  
  displayChinese(20, 80, ssidText.c_str(), ST77XX_BLACK);
  displayChinese(20, 100, pwdText.c_str(), ST77XX_BLACK);
  displayChinese(20, 120, "IP: 192.168.4.1", ST77XX_BLACK);
  displayChinese(20, 140, "Open Browser:", ST77XX_BLACK);
  displayChinese(20, 160, "http://192.168.4.1", ST77XX_BLACK);
...
void connectWiFiAndSyncTime() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    tft.fillScreen(CYBER_BG);
    displayChinese(20, 110, "WiFi Config Failed!", ST77XX_RED);
    delay(1000);
  }
}
